    ...
    (cond
     ...
     ((pair? expr)
      (case (car expr)
        ...
        ((lambda)
         (let ((old-params (cadr expr))
               (body-exprs (cddr expr)))
           (if (params-variadic? old-params)
               (let* ((old-fixed (params-fixed old-params))
                      (old-rest (params-rest old-params))
                      (new-fixed (map fresh-name old-fixed))
                      (new-rest (fresh-name old-rest))
                      (new-env (append (map (lambda (old new) (list old new))
                                            old-fixed new-fixed)
                                       (list (list old-rest new-rest))
                                       env))
                      (body-unique
                       (map (lambda (body-expr) (uniquify-expr body-expr new-env))
                            body-exprs)))
                 `(lambda ,(make-params new-fixed new-rest) ,@body-unique))
               (let* ((unique-params (map fresh-name old-params))
                      (new-env (append (map (lambda (old new) (list old new))
                                            old-params unique-params)
                                       env))
                      (body-unique
                       (map (lambda (body-expr)
                              (uniquify-expr body-expr new-env))
